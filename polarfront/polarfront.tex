...
Surprisingly, increasing read counts only slightly decreased the rate, at both low and high assemblage ratios.
...
It is worth noting that the assemblage ratio is not an inherent property of the assemblage (although it is limited by the assemblage's species richness); rather, it can be decreased (and thus the accuracy of the metagenomic experiment improved) by performing \softwarename{blast} searches against larger databases which have finer taxonomic resolution.
