...
\subsection{Differential influence of advection on different OTUs}
...
\softwarename{bvstep} seeks to find a subset from a community of \acp{OTU}, the inter-sample resemblence pattern of which best matches that of the entire community.
...
The algorithm proceeds through successive rounds of forward and backward selection, beginning with a randomly selected subset of six \acp{OTU}. 
...
In the backward selection round, each \ac{OTU} currently in the subset is tested as a candiate for removal.
...
This allows an estimation of the ``structual redunancy'' of the community: the number of potential subsets of \acp{OTU} which can be extracted from the total set without replacement.
...
Instead, the Bray-Curtis resemblence matrix between samples is generated using this subset, and compared to the advection distance matrix using a partial Mantel test holding the spatial and environmental distances constant.
...
Additionally, after the random selection of six \acp{OTU} to form the starting subset, only the single \ac{OTU} with the highest correlation was retained.
...
Removing five of the starting \acp{OTU} immediately thus eliminated many unnecessary rounds of selection.
