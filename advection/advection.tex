...
The Baas Becking hypothesis --- that ``everything is everywhere, but, the environment selects \cite{BaasBecking:1934um, deWit:2006de}'' --- posits that the rapid dispersal of microorganisms means microbial community structure is determined entirely by environmental selection.
...
Microbial biogeography studies have begun to show that historical factors may also shape the distribution of microorganisms \cite{Martiny:2006jy}, e.g.\ a correlation between spatial and genetic distance (a ``distance effect'') in fluorescent \geuns{Pseudomonas} strains in soils \cite{Cho:2000tn}.
This study, among others \cite{Ramette:2007bb, Storch:ix}, also demonstrated the importance of taxonomic resolution in describing such biogeographic patterns.
...
TODO add mention of appendix on PF sinking results

One explanation for these results is that microbial assemblages are influenced by the advection (physical transport) of cells by ocean currents. 
...
Hence, it would be expected that locations that are closely connected by advection would have more similar compositions than those that are not, even when the environment effect is accounted for.
Indeed, advection is often invoked to explain observations of microbial diversity or abundance which do not seem attributable to environmental selection \citep[e.g.][]{Sul:2013in, Ghiglione:2012cm, Giebel:2009hr, Lauro:2007bf}).
The exchange of very small volumes of water between marine microbial mesocosms has been found to greatly reduce their \textbeta-diversity (in this case, compositional differences between communities from different mesocosms) even under consistent environmental conditions \cite{Declerck:2013cz}.
...
The \ac{SO} is composed of several water masses, which are physicochemically distinct but linked by circulation \cite{Foldvik:1988gp} (TODO ref to sample map). TODO crossref to introduction
TODO Steve wrote a lot of this paragraph - rewrite
Oxygen-depleted \ac{UCDW}, originating in the deep Indian and Pacific oceans \cite{Orsi:1995va}, and high salinity \ac{LCDW} originating in the Atlantic, spread poleward and shoal to reach the sea surface between the Polar Front and the Antarctic continent.
Some of the upwelled \ac{CDW} moves southwards, where it becomes colder and denser and sinks to form \ac{AABW}, which spreads northwards to ventilate the densest layers of the global ocean.
The remainder of the upwelled water is warmed and freshened as it moves northward before sinking to form \ac{AAIW}, which spreads northwards at about 1000 m depth.
Overlying the \ac{AAIW} is the warmer \ac{SAMW}.
