...
The \ac{dbRDA} plot showed the six variables retained by the \ac{distLM} model (i.e.\ all except phosphate) structured the samples first along an axis separating surface and deep samples (dbRDA1), strongly related to dissolved oxygen (r = 0.79) (TODO insert and ref to dbRDA plot).
The second axis was best correlated with temperature (r = \textminus{}0.75).
All six retained variables had a moderate correlation with at least one of the first two axes (TODO insert and ref to table of dbRDA values).
As with the \ac{nMDS} ordinations (TODO ref to all nMDS figures), the water masses were generally well separated by the first two \ac{dbRDA} axes.
Samples from the \ac{AABW} and \ac{AAIW}, which were not separated by the first two axes, were clearly separated along the third (TODO ref to dbRDA values table), which was best correlated with pressure (r = \textminus{}0.79).
